...
app.post('/user', (req, res, userName, pass, email) => {
	...
	//SQL autoincrement
	//var userID =
	.query('INSERT INTO userAccount VALUES (userName, pass, email)');
});

//USER STORY 12
app.get('/user/:userid', (req, res, User, Pass) => {
	.query('SELECT * FROM userAccount WHERE userName = ' + User + ' AND userPassword = ' + Pass,
		function(err, row) { 
...
app.put('/passUpdate/:userid', (req, res, user, newPass) => {
	.query('UPDATE userAccount SET userPassword = ' + newPass + ' WHERE userName = ' + user);
});


//USER STORY 31
app.post('/user/:userid/pantry/add', (req, res, pID, uID, fName, fGroup, exDate, quant, desc, br) => {
	.query('INSERT INTO pantry VALUES (' + pID + ', ' + uID + ', ' + fName ', ' + fGroup ', ' + exDate ', ' + quant ', ' + desc + ', ' + br + ')');
});
...
app.put('/user/:userid/pantry/item/exDate', (req, res, uID, exDate) => {
	.query('UPDATE pantry SET expirationDate = ' + exDate + ' WHERE userID = ' + uID + ' AND foodName = ' + fName);
});

//USER STORY 42
app.delete('/user:userid/pantry/item', (req, res, uID, fName) => {
	.query('DELETE FROM pantry WHERE foodName = ' + fName + ' AND userId = ' + uID);
});

app.post('/user:userid/pantry/favorite', (req, res, fName, uID) => {
	.query('INSERT INTO favoriteFood VALUES (' + uID + ', ' + fName + ')');
});

//USER STORY 13
app.get('/user/:userid/groceryList', (req, res, uID) => {
	.query('SELECT * FROM groceryList WHERE userid = ' + uID);
});
...
app.delete('user/:userid/groceryList/item', (req, res, uID, fName, fGroup, brand, quant) => { //Quantity either needs to be specified upon saying it was bought or it will be defaulted to 1
	.query('INSERT INTO pantry VALUES(' + uID + ', ' + fName + ', ' + fGroup + ', ' + brand + ', ' + quant + ')');
	.query('DELETE FROM groceryList WHERE foodName = ' + fName ' AND userID = ' + uID);
});

//USER STORY 18
app.post('user/:userid/groceryList', (req, res, uID, fName, b, quant) => {
	.query('INSERT INTO groceryList VALUES(' + uID + ', ' + fName + ', ' + b + ', ' + quant + ')');
});

//USER STORY 24
app.get('user/:userid/pantry/categories', (req, res, uID) => {
	.query('SELECT * FROM pantry WHERE userID = ' + uID + ' GROUP BY foodGroup');
});

//USER STORY 24
app.get('user/:userid/pantry', (req, res, uID) => {
	.query('SELECT * FROM pantry WHERE userID = ' + uID);
});

//USER STORY 30
app.get('user/:userid/pantry/exp', (req, res, uID) => {
	.query('SELECT * FROM pantry WHERE userID = ' + uID + 'ORDER BY (expirationDate) ASC');
});


app.get('user/:userid/recipes/ingredient', (req, res, uID, ingred) => {
	.query('SELECT * FROM recipes NATURAL JOIN recipeAssignments NATURAL JOIN foodItem WHERE userID = ' + uID + ' AND foodName = ' + ingred);
});

//Viewing list of recipes for a user
app.get('user/:userid/recipes', (req, res, uID) => {
	.query('SELECT recipeName FROM recipes WHERE userID =' + uID);
});
